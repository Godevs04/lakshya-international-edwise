...
function measureRowHeight(
  ...
  columns.forEach((column, index) => {
    const lines = wrapText(
      doc,
      humanizeCell(column, row[column] ?? ""),
      columnWidths[index],
      fontSize
    );
...
function drawPageFooter(doc: jsPDF, pageNumber: number, totalPages: number, companyName: string) {
...
function drawReportHeader(
  ...
  doc.text(
    `Generated on: ${format(generatedAt, "dd MMM yyyy, hh:mm a")}`,
    margin + 4,
    metaTop + 4.8
  );
...
function drawTableSection(
  ...
  let headerHeight = drawTableHeader(
    doc,
    columns,
    columnWidths,
    margin,
...
export function exportToCsv(
  ...
  for (const row of rows) {
    lines.push(
      columns.map((column) => escapeCsvCell(formatSpreadsheetValue(column, row[column]))).join(",")
    );
  }
...
export function exportToExcel(
  ...
  applyExcelPresentation(worksheet, columns, rows, headerRowIndex, options.subtitle ? 1 : 0);
...
export async function exportToPdf(
  ...
  columnChunks.forEach((columns, chunkIndex) => {
    const sectionLabel =
      columnChunks.length > 1 ? `Section ${chunkIndex + 1} of ${columnChunks.length}` : undefined;
    ...
    y = drawTableSection(doc, ctx, columns, rows, y, sectionLabel, chunkIndex > 0);
  });
