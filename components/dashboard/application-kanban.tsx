  ...
  return (
    <div ref={setNodeRef} style={style} {...(canWrite ? { ...attributes, ...listeners } : {})}>
      ...
      >
        ...
        <p className="mt-2 text-base font-bold text-[#0B8FD8]">{formatCurrency(app.loanAmount)}</p>
...
function KanbanColumnInner({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`min-h-[200px] max-h-[min(70vh,720px)] overflow-y-auto rounded-2xl border border-[#0B8FD8]/10 p-2 transition-all ${isOver ? "bg-[#0B8FD8]/10 ring-2 ring-[#0B8FD8]/20" : "bg-white/40 backdrop-blur-sm dark:bg-white/5"}`}
    >
      {apps.map((app) => (
