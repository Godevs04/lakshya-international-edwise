...
import { TARGET_COUNTRIES, TARGET_DEGREES, TARGET_INTAKES } from "@/lib/constants/study-abroad";
...
export function StudentForm({
  ...
  const [targetUniversity, setTargetUniversity] = useState(
    (initialData?.targetUniversity as string) ?? ""
  );
  ...
  const commissionPreview = useMemo(() => {
    ...
    partnerShareOverride,
    selectedPartner?.commissionPercent,
  ]);
          ...
          <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="mt-1.5">
                  <InputGroup className="h-9">
                    <InputGroupAddon align="inline-start" className="border-r border-input pr-1">
                      <Select
                        value={loanCurrency}
                        onValueChange={(v) => setLoanCurrency(v ?? "INR")}
                      >
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <p className="text-xs text-muted-foreground">
                Your commission rate from the lender for this student. Amounts calculate
                automatically from disbursement.
              </p>
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="commissionPercentOverride">
                Partner Share Override % (of disbursement)
              </Label>
              <Input
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {!partnerId
                  ? " Select a consultancy in the first section to enable this field."
                  : null}
              </p>
            </div>
            {Number(loanDisbursed) > 0 && Number(ourCommissionPercent) > 0 ? (
              <div className="rounded-xl border border-[#0B8FD8]/15 bg-[#0B8FD8]/5 p-3 text-sm sm:col-span-2">
